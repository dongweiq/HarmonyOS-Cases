...
在出行类订票软件中，一般都有此动画效果，点击交换后，起点和终点互换。地址交换动画的方案可以参考此[详细说明文档](./feature/addressexchange/README_zh.md)。

#### 应用启动页和开屏广告方案

在应用开发时，基本上都需要一个应用的启动页，包含logo和应用的说明语。根据需求在启动页后，展示广告内容，用户任意时刻可点击跳转进入首页。应用启动页和开屏广告的方案可以参考此[详细说明文档](./feature/launchscenes/README_zh.md)。
